    ...
    execute: function () {
      ...
      _export("wordContorl", wordContorl = (_dec = ccclass('wordContorl'), _dec2 = property(Node), _dec3 = property({
        ...
        generateWord() {
          ...
          for (var j = 0; j < this.nowIndex + 1; j++) {
            for (var i = 0; i < this.texts[j].length; i++) {
          ...
          wordArray.forEach(element => {
            var word = instantiate(this.word);
            word.name = element;
        ...
        addEvent() {
          var addEventFunc = (start, end, tag) => {
            console.log(tag);
            var wordArray = [];
            ...
            for (var i = 0; i < wordArray.length; i++) {
              if (i >= start && i < end) {
                ...
                var tangmianScript = this.node.children[i].getComponent(_crd && tangmian === void 0 ? (_reportPossibleCrUseOftangmian({
                  error: Error()
                }), tangmian) : tangmian);
          ...
          var count = 0;
          var quotesPositions = []; // 最终的数组，存储每对引号的位置

          var texts = [];

          for (var i = 0; i < this.nowIndex + 1; i++) {
            texts = texts.concat(this.texts[i].split(''));
            console.log(texts);
          }

          console.log(texts + 'texts');
          var tempArray = []; // 临时数组，存储当前字符串中每对引号的位置
...
//# sourceMappingURL=84c3498396c9bf7fae96ada385f73879bb70583a.js.map