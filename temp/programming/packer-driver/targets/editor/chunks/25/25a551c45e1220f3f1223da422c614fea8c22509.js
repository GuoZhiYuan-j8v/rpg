  ...
  return {
    ...
    execute: function () {
      ...
      _export("dialog_galgame", dialog_galgame = (_dec = ccclass('dialog_galgame'), _dec2 = property(AudioSource), _dec3 = property(AudioClip), _dec4 = property([EventHandler]), _dec5 = property({
        ...
        resizeBackground() {
          // 获取屏幕可见区域尺寸
          const screenSize = view.getVisibleSize(); // 获取图片原始尺寸

          const spriteFrame = this.bg_sprite.spriteFrame;
          const textureSize = spriteFrame == null ? void 0 : spriteFrame.originalSize;
          if (!textureSize) return; // 计算屏幕与图片的宽高比

          const screenRatio = screenSize.width / screenSize.height;
          const imageRatio = textureSize.width / textureSize.height; // 计算适配比例
            ...
            if (this.cell[0] == '&') {
              ...
              if (condition) {
                if (condition.trim()) {
                  ...
                  if ((_crd && dialogLocalData === void 0 ? (_reportPossibleCrUseOfdialogLocalData({
                    error: Error()
                  }), dialogLocalData) : dialogLocalData).getSysLocalAboutDialogDOM(condition)) {
...
//# sourceMappingURL=25a551c45e1220f3f1223da422c614fea8c22509.js.map